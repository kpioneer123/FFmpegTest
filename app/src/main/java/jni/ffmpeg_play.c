...
uint8_t *buffer;
	...
	int numBytes = av_image_get_buffer_size(AV_PIX_FMT_RGBA,videoWidth, videoHeight,
		1);
...
void* player_read_from_stream(void* arg) {
	...
	for (;;) {
		...
		//根据AVpacket->stream_index获取相应的队列
		Queue *queue = player->packets[packet.stream_index];

	    QueuePush(queue,packet,&player->mutex,&player->cond,0);

		//		LOGI("queue:%#x, packet:%#x",(unsigned int)queue,packet);
	}
		pthread_mutex_unlock(&player->mutex);
	LOGI("player_read_from_stream end");
	return 0;
}
